"""
融合数据训练（0填充版）：视觉+手表 vs 纯视觉
输入：D:\DIPSER\sequences_fused\ + C:\DIPSER\ 划分文件
不剔除任何subject
3种子，验证集选阈值，测试集评估一次
"""
import os
...
MODEL_SAVE_PATH = r"C:\DIPSER\fused_model_best.pt"
...
class GRUClassifier(nn.Module):
    def __init__(self, input_dim=1558, hidden_dim=128, num_layers=2, num_classes=2, dropout=0.3):
        super().__init__()
...
def standardize(X_train, X_val, X_test):
    """视觉和手表特征分别标准化"""
    vis_train = X_train[:, :, :VIS_DIM]
    vis_mean = vis_train.mean(dim=(0, 1), keepdim=True)
    vis_std = vis_train.std(dim=(0, 1), keepdim=True) + 1e-8

    watch_train = X_train[:, :, VIS_DIM:]
    watch_mean = watch_train.mean(dim=(0, 1), keepdim=True)
    watch_std = watch_train.std(dim=(0, 1), keepdim=True) + 1e-8

    def transform(X):
        vis = (X[:, :, :VIS_DIM] - vis_mean) / vis_std
        watch = (X[:, :, VIS_DIM:] - watch_mean) / watch_std
        return torch.cat([vis, watch], dim=2)
...
def run_once(seed):
    ...
    X_train, y_train, skip_train = load_split('train_subjects.txt')
    X_val, y_val, skip_val = load_split('val_subjects.txt')
    X_test, y_test, skip_test = load_split('test_subjects.txt')

    if X_train is None or X_val is None or X_test is None:
        print(f"Seed {seed}: 数据加载失败")
        return None
    ...
    model = GRUClassifier(input_dim=1558).to(DEVICE)
    ...
    print(f"Seed {seed}: 训练{len(y_train)}, 验证{len(y_val)}, 测试{len(y_test)}")
    print(f"  阈值={best_thresh:.2f}, 准确率={acc:.4f}, 不参与F1={f1_0:.4f}, "
          f"召回={rec_0:.4f}, 精确={prec_0:.4f}, AUC={auc:.4f}")

    return [acc, f1_0, rec_0, prec_0, auc]


print("融合数据训练（0填充版）：视觉+手表 (1558维)")
print("=" * 60)
...
if results:
    ...
    print(f"\n对比纯视觉（全特征A配置）:")
    print(f"  准确率:     0.7721 ± 0.0071")
    print(f"  不参与F1:   0.4979 ± 0.0113")
    print(f"  不参与召回: 0.5352 ± 0.0407")
    print(f"  不参与精确: 0.4676 ± 0.0117")
    print(f"  AUC:        0.7630")
