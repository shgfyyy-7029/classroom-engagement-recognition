...
7种特征组合（3种子，验证集选阈值，EPOCHS=50）。完整结果见 `ablation/results/ablation_results.md`。

| 组合 | 维度 | 准确率 | 不参与F1 | 不参与召回率 |
|------|------|--------|----------|-------------|
| **full** | 1544 | **77.21%** | **0.4979** | 53.52% |
| headpose_body | 102 | 71.46% | 0.5087 | **69.74%** |
| body_pose_only | 99 | 69.40% | 0.4820 | 67.29% |
| facemesh_body | 1533 | 75.85% | 0.4772 | 53.11% |
| facemesh_head | 1437 | 73.47% | 0.4586 | 53.20% |
| headpose_only | 3 | 75.52% | 0.4563 | 49.21% |
| facemesh_only | 1434 | 76.25% | 0.4473 | 45.42% |

**主要发现**：

1. **full配置综合最优**：准确率最高（77.21%），不参与F1接近最高（0.4979）。
2. **headpose_body不参与召回率最高**（69.74%），适合"宁可多抓"的场景。
3. **面部网格单独使用效果有限**：不参与F1仅0.4473。
4. **特征选择取决于应用目标**：要整体准确选full，要少漏走神选headpose_body。
