...
class GRUSingleFrame(nn.Module):
...
def run_once(seed):
    ...
    model = GRUSingleFrame(input_dim=1544).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"C:\\DIPSER\\singleframe_seed{seed}.pt")
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(f"C:\\DIPSER\\singleframe_seed{seed}.pt"))
