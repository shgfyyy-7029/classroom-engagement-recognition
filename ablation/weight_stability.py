...
CONFIGS = {
    'A_无处理': {'use_cw': False, 'use_sampler': False},
    'B_仅类别权重': {'use_cw': True, 'use_sampler': False},
}
...
def run_once(config_name, use_cw, use_sampler, seed):
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"C:\\DIPSER\\{config_name}_seed{seed}.pt")
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(f"C:\\DIPSER\\{config_name}_seed{seed}.pt"))
...
for name, cfg in CONFIGS.items():
    ...
    results = []
    for seed in SEEDS:
        results.append(run_once(name, cfg['use_cw'], cfg['use_sampler'], seed))
...
for name, results in all_results.items():
    ...
    print(f"\n{name}:")
    print(f"  准确率:     {means[0]:.4f} ± {stds[0]:.4f}")
    print(f"  不参与F1:   {means[1]:.4f} ± {stds[1]:.4f}")
    print(f"  不参与召回: {means[2]:.4f} ± {stds[2]:.4f}")
    print(f"  不参与精确: {means[3]:.4f} ± {stds[3]:.4f}")
    print(f"  参与F1:     {means[4]:.4f} ± {stds[4]:.4f}")
    print(f"  最优阈值:   {means[5]:.2f} ± {stds[5]:.2f}")

with open(r"C:\DIPSER\weight_ablation_stability.txt", 'w') as f:
    f.write("A/B配置三种子稳定性验证\n")
    f.write("=" * 50 + "\n\n")
    for name, results in all_results.items():
        means = results.mean(axis=0)
        stds = results.std(axis=0)
...
print(f"\n结果已保存: C:\\DIPSER\\weight_ablation_stability.txt")
